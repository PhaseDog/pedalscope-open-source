...
% plain-transfer.tex, parity-transfer.tex, lattice-imd.tex, plain-imd.tex,
% parity-imd.tex); nothing numeric is typed here,
...
This chapter is not yet written; the technical reference's Compression
chapter is, and this page names what its plain-language counterpart will
explain, in the words the glossary already defines. A
\term{compression-curve} plays one note at every loudness of a
\term{probe-grid} in turn, with a \term{fine-window} of closer steps
where the user asks for one; the \term{stepped-tone} climbs the ladder
with a \term{settle-window} at every rung, its measured stretch trimmed
by the \term{cycle-snap} so the \term{one-bin-estimator} reads the note
exactly. The silence before the note is the \term{noise-stamp}, and the
\term{noise-gate} leaves out rungs where the note is lost in the
hiss. From the curve the app reads the \term{incremental-slope} between
rungs and fits a \term{hinge-fit} --- two straight lines with a corner
--- whose corner is the \term{knee}: a number with a \term{resolution-band}
when the quiet end is clean, or ``at or below'' when the \term{unity-guard}
finds the pedal already squashing at the quietest rung or the corner sits
in the \term{bound-zone}. The \term{cleanup-reading} says where the
distortion first crosses the line, or that it never does, or that it
always is. The \term{noise-bound} says how much distortion the hiss alone
could fake at each rung, a \term{null-run-reference} says how much the
loop itself adds, the \term{floor-source} names which one the chart
drew, the \term{floor-class} says of each rung whether its reading is
the pedal, a mix, or the hiss, and the \term{floor-runs} are the stretches
drawn solid, dotted or faint. The \term{summed-presence} checks
that the note came through at all before any of that is quoted. The lay
chapter will explain each of those in turn, with the technical chapter's
figures.
